...
to CLASSIFY-OVERALL-SEVERITY
  ...
  [
    ...
    foreach tier-1 [ x -> ask x [set overall-severity-category "tier 1"] ]
    foreach tier-2 [ x -> ask x [set overall-severity-category "tier 2"] ]
    foreach tier-3 [ x -> ask x [set overall-severity-category "tier 3"] ]
  ]
...
to SET-SUMMER-CROP-ECONOMICS
  ...
  if economy? = "Canada: Canola(S), Wheat(W)"
  [
    ;;summer canola (Prairie Alberta/Saskatchewan)
    set crop-price 0.55
    set IWA 5.0
    set crop-yield 3000
...
to SETUP-EXPERIMENT
  ...
  SETUP-PV  ;; initialize has-pv? to false (pv-adoption-fraction=0 after ca); caller can re-invoke with desired fraction
  reset-ticks
  set year 0
end
