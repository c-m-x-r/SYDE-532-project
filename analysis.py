"""
Analysis and plotting for Groundwater Commons Game paper-protocol output.

CSV format (paper_protocol): one tidy row per (run, tick), produced by run_paper.py.
"""
...
DISPLAY_SLICE   = (0, 100)   # full 100-year trajectory: ticks 0-49 lax + ticks 50-99 enforcement
...
# ---------------------------------------------------------------------------
# PV comparison plot (agrivoltaic extension — parked, run_pv.py not integrated)
# ---------------------------------------------------------------------------
...
#     NOTE: run_pv.py produces results/pv_freemarket.csv; align naming before re-enabling.
#     """
...
#                 mean, std = g.mean(), g.std()
#                 ax.plot(mean.index, mean.values, linestyle=ls, color=color, linewidth=1.2,
